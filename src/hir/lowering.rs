...
//         let sliding_essences = self.ir.sliding_windows.iter().map(|window| (window.target, window.reference));
//         // let discrete_essences = self.ir.discrete_windows.iter().map(|window| (window.target, window.reference));
...
//                     let op = *aggregation;
//                     let reference = WindowReference::SlidingWindow(self.ir.sliding_windows.len());
//                     let ty = self.lower_node_type(win_expr.id);
//                     let window = SlidingWindow { target, duration, wait: *wait, op, reference, ty };
