use super::*;
use num::Signed;
use reporting::{Handler, LabeledSpan};
...
impl<'a, 'b> Verifier<'a, 'b> {
    fn new(spec: &'a RTLolaAst, handler: &'b Handler) -> Self {
        Self { spec, handler }
    }
    ...
    fn check_expression(&self, expr: &Expression) {
        expr.iter()
            .for_each(|inner| Self::check_missing_paranthesis(self.handler, inner));
        expr.iter()
            .for_each(|inner| Self::check_missing_expression(self.handler, inner));
        expr.iter()
            .for_each(|inner| Self::check_direct_access(self.handler, inner));
        expr.iter()
            .for_each(|inner| Self::check_field_access(self.handler, inner));
        expr.iter()
            .for_each(|inner| Self::check_valid_offset(self.handler, inner));
        expr.iter()
            .for_each(|inner| Self::check_discrete_window_duration(self.handler, inner));
        expr.iter()
            .for_each(|inner| Self::check_sliding_window_duration(self.handler, inner));
    }
    ...
    fn check_valid_offset(handler: &Handler, expr: &Expression) {
        ...
        if let Offset(_, offset) = &expr.kind {
            if let super::Offset::Discrete(val) = offset {
                if *val == 0 {
                    handler.error_with_span(
                        "only non-zero offsets are permitted",
...
mod tests {
    ...
    fn number_of_warnings(content: &str) -> usize {
        ...
        let ast =
            parse(content, &handler, FrontendConfig::default()).unwrap_or_else(|e| panic!("{}", e));
    ...
    fn test_offsets_direct_access() {
        ...
        assert_eq!(
            1,
            number_of_errors("output a := (x+1).aggregate(over: 1h, using: avg)")
        );
    }
}
